...
func NewSUNATScraper() (*SUNATScraper, error) {
	...
	url := l.Bin(os.Getenv("ROD_BROWSER_PATH")).MustLaunch()
	browser := rod.New().
		ControlURL(url).
		MustConnect()
...
func (s *SUNATScraper) ScrapeRUC(ruc string) (*models.RUCInfo, error) {
	...
	// Check if we need to handle any popups or new windows
...
func (s *SUNATScraper) mapFieldToStruct(label, value string, info *models.RUCInfo) {
	...
	case strings.Contains(label, "comprobantes electrónicos") || strings.Contains(label, "comprobantes electronicos"):
		// Store as single string since it's a comma-separated value
