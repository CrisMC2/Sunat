...
# Inicializar contadores con verificación
if [ -n "$STATS_FILE" ] && [ -w "$(dirname "$STATS_FILE")" ] 2>/dev/null; then
    ...
    print_error "Error: No se puede escribir archivo de estadísticas: $STATS_FILE"
    exit 1
fi
...
print_info "Ver errores terminales: psql '$DATABASE_URL' -c 'SELECT COUNT(*) FROM log_consultas WHERE estado = \"error_terminal\";'"
    ...
    # Verificación final
    local remaining=$(pgrep -c -f "chrome\|chromium" 2>/dev/null || echo "0")
    if [ "$remaining" -gt 0 ]; then
        print_warning "Aún quedan $remaining procesos de Chrome/Chromium"
        # Último intento más agresivo
        pkill -9 -f "chrome" 2>/dev/null || true
...
worker_function() {
    ...
    # Registrar el PID del proceso Go para poder matarlo después
    echo "$go_pid" >> "${ACTIVE_PIDS_FILE}.go_processes" 2>/dev/null || true
    ...
    # Leer salida con verificación de archivos
    local output=""
    local error_output=""
    if [ -r "$temp_output" ]; then
    ...
    case $exit_code in
        ...
        124)
            ...
            ;;
        *)
            # MOSTRAR EL ERROR COMPLETO EN LA TERMINAL PARA DEBUGGING
            if [ -n "$error_output" ]; then
                print_worker "$worker_id" "ERROR COMPLETO: $error_output"
            fi
            if [ -n "$output" ]; then
                print_worker "$worker_id" "STDOUT: $output"
            fi
            
            # Preparar mensaje de error para la BD (limitado)
            local error_msg="Error $exit_code"
            if [ -n "$error_output" ]; then
                error_msg="$error_msg: $(echo "$error_output" | head -c 100)"
            fi
...
cleanup() {
    ...
    local go_remaining=$(pgrep -c -f "go run\|scraper-completo" 2>/dev/null || echo "0")
    if [ "$go_remaining" -gt 0 ]; then
        print_warning "Quedan $go_remaining procesos Go. Terminación final..."
        pkill -9 -f "go run" 2>/dev/null || true
        pkill -9 -f "scraper-completo" 2>/dev/null || true
    ...
    local chrome_remaining=$(pgrep -c -f "chrome\|chromium" 2>/dev/null || echo "0")
    if [ "$chrome_remaining" -gt 0 ]; then
        print_warning "Quedan $chrome_remaining procesos Chrome. Terminación final..."
        pkill -9 -f "chrome\|chromium" 2>/dev/null || true
    fi
